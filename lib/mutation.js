...
var mutation = function () {
  ...
  var m = arguments[0];
  var mType = typeChecker.getType(m);
  if (!typeChecker.isObject(m)) {
    throw new Error(errorsManager.constructorArguments('mutation', 'object', mType));
  }
...
mutation.checkValueType = function(allowedTypes, value, methodName, index) {
  ...
  if (allowedTypes.indexOf(valueType) === -1) {
    throw new Error(errorsManager.argumentTypes(allowedTypes, valueType, methodName, index));
  }
};
...
mutation.prototype.parseMutation = function (json) {
  ...
  keys.forEach(function(key) {
    ...
    if (typeChecker.isObject(json[key])) {
      ...
      throw Error(errorsManager.invalidNotation('mutation', key, json[key]));
    }
