...
var makeDocumentsStream = function(jDocs, eventHandlers) {
  ...
  // when document recived push it to callback
...
Table.prototype.findById = function() {
  ...
  args.addCombo('id', 'fields', 'callback');
  args.addCombo('id', 'callback');

  args.inspect('callback', typeChecker.isFunction.bind(typeChecker));
  args.inspect('id', function(i) {
  ...
  args.castTo('callback', function(originalCallback) {
  ...
  if (args.list() === 3) {
    return this.jTable.findById.apply(this.jTable, inlineArgs(args.list(), 2));
  } else {
    return this.jTable.findById.apply(this.jTable, args.list());
  }
...
Table.prototype.find = function() {
  ...
  if (args.list().length) {
    ...
    if (args.list().length === 3) {
      return this.jTable.find.apply(this.jTable, inlineArgs(args.list(), 2));
    }
...
Table.prototype.eachDocument = function(condition, fields, callback) {
  ...
  args.castTo('callback', function(i) {
    return findCallbackWrap(i, true);
  });
  args.castTo('condition', function(i) {
    return new Condition(i);
  });
...
Table.prototype.stream = function() {
  ...
  args.addCombo('condition', 'fields', 'streamHandlers');
  args.addCombo('condition', 'streamHandlers');
  args.addCombo('fields', 'streamHandlers');

  args.inspect('condition', function(i) {
    if (typeChecker.isObject(i)) {
      return !Object.keys(i).filter(function(key) { return typeChecker.isFunction(i[key]); }).length;
    } else if (typeChecker.checkEachType(i, 'object')) {
      return true;
    }
    return false;
  });
  ...
  args.inspect('streamHandlers', function(i) {
    return !!Object.keys(i).filter(function(key) { return typeChecker.isFunction(i[key]); }).length;
  });
  args.castTo('condition', function(i) {
    return new Condition(i);
  });
  args.castTo('streamHandlers', function(streamHandlers) {
