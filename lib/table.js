...
var makeDocumentsStream = function(jDocs, eventHandlers) {
  ...
  var errHandlerWrap = function(eventCallback) {
    return function(err) {
      if (typeChecker.isFunction(eventCallback)) {
        eventCallback(err);
      }
      emitter.emit('end');
    };
...
/**
 * Find document by specified id.
 *
 * @param {string} id document id
 * @param {string[]} [fields] returning document fields
 * @returns {object}
 */
...
/**
 * Scan documents by specified condition and return specified fields only.
 *
 * @param {object|object[]} [condition] condition to filter documents
 * @param {string[]} [fields] returning document fields
 * @param {function} callback callback function. error and docs will be passed
 */
...
/**
 * Iterate over stream of documents by specified condition with specified fields.
 * <code>streamHandlers</code> object is required and at least <code>condition</code> JSON object
 * or <code>fields</code> list.
 *
 * @param {object|object[]} [condition] condition to filter documents
 * @param {string[]} [fields] document field list
 * @param {object} streamHandlers event subscriber with following structure
 * {
 *   err: function(err) {
 *     // handle error if appears
 *   },
...
/**
 * Close table resource. Should be called when you've done your work with table.
 * This will prevents memory leaks issues.
 * If no callback passed then this method calls synchronously, in other case asynchronously
 *
 * @param {function} [callback]
 * @returns {*}
 */
Table.prototype.close = function(callback) {
  return callback ?
    this.jTable.close(callback) :
    this.jTable.closeSync();
};
