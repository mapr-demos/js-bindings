...
module.exports = {
  ...
  invalidNotationError: function (klass, key, obj) {
    return new InvalidNotationError('Invalid "' + klass + '"-field for key "' + key + '". Should be an object like "{field: {$cond: \'state\'}}" but got "' + typeChecker.getType(obj) + '".');
  },
  ...
  argumentTypesWhiteListError: function (allowedTypes, valueType, methodName, index) {
    ...
    return new ArgumentTypesWhiteListError('"' + valueType + '" is not allowed for "' + methodName + '" ' + digitToSequenceNumber(index) + ' argument. Only ' + toComaList(allowedTypes) + (allowedTypes.length === 1 ? ' is' : ' are') + ' allowed!');
  },
  ...
  argumentTypesBlackListError: function (disallowedTypes, methodName, index) {
    ...
    return new ArgumentTypesBlackListError(toComaList(disallowedTypes) + ' ' + (disallowedTypes.length === 1 ? ' is' : ' are') + ' not allowed for "' + methodName + '" ' + digitToSequenceNumber(index) + ' argument.');
  },
  ...
  notSupportedMethodSignatureError: function (methodName, args, signatures) {
    var argTypes = Array.prototype.slice.call(args || []).map(typeChecker.getType.bind(typeChecker)).join(', ');
    return  new NotSupportedSignatureError('"Wrong method signature: ' + methodName + ' called with [' + argTypes + '] but expect ' + JSON.stringify(signatures));
  },
